...
class CosyVoiceFrontEnd(object):
    ...
    def extract_speech_feat(self, audio:torch.Tensor, audio_sr:int):
        if audio_sr != self.sample_rate:
            audio = librosa.resample(audio.cpu().numpy(), orig_sr=audio_sr, target_sr=self.sample_rate)
    ...
    def extract_spk_embedding(self, audio:torch.Tensor, audio_sr:int):
        if audio_sr != 16000:
            audio = librosa.resample(audio.cpu().numpy(), orig_sr=audio_sr, target_sr=16000)
            audio = torch.from_numpy(audio).float()
            audio_sr = 16000
